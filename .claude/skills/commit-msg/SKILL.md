...
Check the current branch first with `git branch --show-current`. If it is `main`, stop and tell the user changes should go through a feature branch and PR instead (see the `pr-description` skill) -- release notes are generated from merged PRs, so direct commits to `main` won't show up in the release notes.

When committing changes on a feature branch:
